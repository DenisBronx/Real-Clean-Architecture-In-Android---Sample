package com.denisbrandi.androidrealca.plp.presentation.view.previews

import com.denisbrandi.androidrealca.money.domain.model.Money
import com.denisbrandi.androidrealca.product.domain.model.Product

val productList = listOf(
    Product(
        "1",
        "Wireless Headphones",
        Money(99.99, "$"),
        "https://m.media-amazon.com/images/I/61fU3njgzZL._AC_SL1500_.jpg"
    ),
    Product(
        "2",
        "Smartphone Stand",
        Money(15.49, "$"),
        "https://m.media-amazon.com/images/I/61shuq7IDcL._AC_SL1500_.jpg"
    ),
    Product(
        "3",
        "Bluetooth Speaker",
        Money(79.99, "$"),
        "https://m.media-amazon.com/images/I/81mr1duG3ZL._AC_SL1500_.jpg"
    ),
    Product(
        "4",
        "Portable Charger",
        Money(24.99, "$"),
        "https://m.media-amazon.com/images/I/61cAlFkKsyL._AC_SL1500_.jpg"
    ),
    Product(
        "5",
        "Wireless Mouse",
        Money(29.99, "$"),
        "https://m.media-amazon.com/images/I/61N+CzcA8vL._AC_SL1500_.jpg"
    ),
    Product(
        "6",
        "USB-C Cable",
        Money(12.99, "$"),
        "https://m.media-amazon.com/images/I/61wYXznLNtL._SL1500_.jpg"
    ),
    Product(
        "7",
        "Laptop Sleeve",
        Money(34.99, "$"),
        "https://m.media-amazon.com/images/I/819Ook6vDGL._AC_SL1300_.jpg"
    ),
    Product(
        "8",
        "Smartwatch",
        Money(199.99, "$"),
        "https://m.media-amazon.com/images/I/71Ggfcmy2cL._AC_SL1500_.jpg"
    ),
    Product(
        "9",
        "Noise Cancelling Earbuds",
        Money(149.99, "$"),
        "https://m.media-amazon.com/images/I/719T7V6DWWL._AC_SL1500_.jpg"
    ),
    Product(
        "10",
        "Gaming Keyboard",
        Money(89.99, "$"),
        "https://m.media-amazon.com/images/I/71wBsNJdfdL._AC_SL1500_.jpg"
    ),
    Product(
        "11",
        "Smart Light Bulb",
        Money(22.49, "$"),
        "https://m.media-amazon.com/images/I/51X4qos6UZL._AC_SL1500_.jpg"
    ),
    Product(
        "12",
        "Fitness Tracker",
        Money(129.99, "$"),
        "https://m.media-amazon.com/images/I/61PTIJf30LL._AC_SL1500_.jpg"
    ),
    Product(
        "13",
        "Digital Camera",
        Money(449.99, "$"),
        "https://m.media-amazon.com/images/I/71S1mWH2EkL._AC_SL1500_.jpg"
    ),
    Product(
        "14",
        "Tablet Stand",
        Money(17.99, "$"),
        "https://m.media-amazon.com/images/I/71rZLuhfn8L._AC_SL1500_.jpg"
    ),
    Product(
        "15",
        "Wireless Charger",
        Money(27.99, "$"),
        "https://m.media-amazon.com/images/I/71Nc1SSWA7L._AC_SL1500_.jpg"
    ),
    Product(
        "16",
        "Action Camera",
        Money(329.99, "$"),
        "https://m.media-amazon.com/images/I/71s04+f9SQL._AC_SL1500_.jpg"
    ),
    Product(
        "17",
        "Smart Thermostat",
        Money(199.99, "$"),
        "https://m.media-amazon.com/images/I/51j3dmQr+dL._AC_SL1163_.jpg"
    ),
    Product(
        "18",
        "Bluetooth Headset",
        Money(49.99, "$"),
        "https://m.media-amazon.com/images/I/61LuyFgTYZL._AC_SL1500_.jpg"
    ),
    Product(
        "19",
        "Laptop Cooling Pad",
        Money(29.99, "$"),
        "https://m.media-amazon.com/images/I/71vY18ciJsL._AC_SL1500_.jpg"
    ),
    Product(
        "20",
        "Smart Doorbell",
        Money(159.99, "$"),
        "https://m.media-amazon.com/images/I/61967NLOAiL._AC_SL1500_.jpg"
    )
)
